...
export function estimateAgendaTalkCardHeightForWidth(
  ...
  const titleLineCount = Math.max(
    1,
    Math.ceil(talk.title.length / approximateCharactersPerLine),
  );
...
function buildTimeAxis(
  ...
  for (let iteration = 0; iteration < 16; iteration += 1) {
    ...
    const boundaryTopPx = buildBoundaryTopMap();
    const blocksByColumn = new Map<number, typeof blocks>();
    blocks.forEach((block) => {
      if (block.columnIndex < 0 || block.isTalk) {
        return;
      }

      const columnBlocks = blocksByColumn.get(block.columnIndex) ?? [];
      columnBlocks.push(block);
      blocksByColumn.set(block.columnIndex, columnBlocks);
    });

    blocksByColumn.forEach((columnBlocks) => {
      const orderedBlocks = [...columnBlocks].sort(
        (left, right) => left.startMinutes - right.startMinutes,
      );

      orderedBlocks.slice(1).forEach((block, index) => {
        const previousBlock = orderedBlocks[index];
        if (!previousBlock) {
          return;
        }

        const previousTopPx =
          boundaryTopPx.get(previousBlock.startMinutes) ?? 0;
        const currentTopPx = boundaryTopPx.get(block.startMinutes) ?? 0;
        const deficit =
          previousTopPx + previousBlock.trackHeightPx - currentTopPx;
        if (deficit <= 0) {
          return;
        }

        const startIndex = boundaries.indexOf(previousBlock.startMinutes);
        const endIndex = boundaries.indexOf(block.startMinutes);
        if (startIndex < 0 || endIndex <= startIndex) {
          return;
        }
        ...
        for (
          ...
          segmentHeights[segmentIndex] =
            (segmentHeights[segmentIndex] ?? 1) + extraPerSegment;
        }
        changed = true;
      });
    });
...
export function getResponsiveAgendaColumnWidth(
  ...
  const targetVisibleColumns = Math.min(
    columnCount,
    columnCount >= 3 ? 3 : viewportWidthPx >= 800 ? 2 : 1.25,
  );
...
export function buildAgendaCanvasLayout(
  ...
  const buildTimeAxisBlocks = (blocks: typeof solvedBlocks) => [
    ...blocks.map((block) => ({
      startMinutes: block.startMinutes,
      endMinutes: block.endMinutes,
      trackHeightPx: block.trackHeightPx,
      columnIndex: block.columnIndex,
      isTalk: false,
    })),
    ...blocks.flatMap((block) =>
      block.talkPlacements.flatMap((placement) => {
        const startMinutes = getAgendaTalkStartMinutes(placement.talk);
        ...
        return [
          {
            startMinutes,
            endMinutes: getAgendaTalkEndMinutes(placement.talk),
            trackHeightPx:
              placement.heightPx +
              agendaCanvasTalkGap +
              (startMinutes === block.startMinutes
  ...
  let axisBlocks = solvedBlocks;
  let timeAxis = buildTimeAxis(
    buildTimeAxisBlocks(axisBlocks),
    markerStart,
    markerEnd,
  );

  let axisAlignedColumns: AgendaCanvasColumnLayout[] = [];
  for (let iteration = 0; iteration < 8; iteration += 1) {
    axisAlignedColumns = axisBlocks.map((block) => {
      const blockTopPx = timeAxis.getTopForMinute(block.startMinutes);
      const talkPlacements = block.talkPlacements.map((placement) => {
        const talkStartMinutes = getAgendaTalkStartMinutes(placement.talk);
        if (talkStartMinutes === null) {
          return placement;
        }
      ...
      const lastTalkBottomPx = talkPlacements.reduce(
        (maxBottom, placement) =>
          Math.max(maxBottom, placement.topPx + placement.heightPx),
        0,
      );
      ...
      return {
        ...
        trackHeightPx: Math.max(
          block.trackHeightPx,
          lastTalkBottomPx + agendaCanvasTrackPadding,
        ),
        blockTopPx,
      };
    });

    const nextAxisBlocks = axisAlignedColumns.map((block) => ({
      ...block,
      trackHeightPx: block.trackHeightPx,
    }));
    const heightsChanged = nextAxisBlocks.some(
      (block, index) =>
        block.trackHeightPx !== axisBlocks[index]?.trackHeightPx,
    );
    axisBlocks = nextAxisBlocks;
    if (!heightsChanged) {
      break;
    }
